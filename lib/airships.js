...
`);
		...
		.then(exists => {
			if (exists) {
				bot.sendMessage(channel, `Airship ${identifier} has been reset.`);
				clearInterval(timers[identifier]);
				airships.delete(identifier);
			} else {
				bot.sendMessage(channel, `Airship ${identifier} is NOT registered.`);
			};
		});
	} else {
		airships
